        ...
        for byte in pattern.split_whitespace() {
            if byte == "?" || byte == "??" {
                ...
                match byte.len() {
                    ...
                    3 | 4 => signature.extend(u16::from_str_radix(byte, 16).map_err(|_| {})?.to_be_bytes().into_iter().skip_while(|b| *b == 0)),
                    5 | 6 => signature.extend(u32::from_str_radix(byte, 16).map_err(|_| {})?.to_be_bytes().into_iter().skip_while(|b| *b == 0)),
                    7 | 8 => signature.extend(u64::from_str_radix(byte, 16).map_err(|_| {})?.to_be_bytes().into_iter().skip_while(|b| *b == 0)),
                    _ => return Err(())
                }
            }
        }
...
unsafe fn get_module_text_section<'a>(module_handle: usize) -> &'a [u8] {
    ...
    if *machine == 0x8664 {
        ...
        slice::from_raw_parts(module_handle as *const u8, nt_headers.OptionalHeader.SizeOfImage as usize)
